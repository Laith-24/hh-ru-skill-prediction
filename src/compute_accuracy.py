...
Defaults to the live results/*.csv files; pass explicit paths (e.g. the
*_final6_unicode.csv files) to score a specific run, and make sure they
match the models currently in models/, so a stale threshold never gets
paired with freshly retrained models. Uses the Column API (F.col/F.avg),
never raw SQL text, since skill names with spaces/Cyrillic break unquoted
SQL identifiers.

Usage: spark-submit src/compute_accuracy.py [--nonspark-results CSV]
    [--spark-results CSV] [--output CSV]
"""
...
parser.add_argument("--nonspark-results", default="results/nonspark_results.csv")
parser.add_argument("--spark-results", default="results/spark_results.csv")
parser.add_argument("--output", default="results/full_metrics_report.csv")
...
try:
    _saved_pipeline = PipelineModel.load(args.pipeline_path)
except Exception:
    logging.warning(f"No saved pipeline at {args.pipeline_path} -- fitting fresh (fine for same-file calls).")
...
nonspark_thresholds = pd.read_csv(args.nonspark_results).set_index(["skill", "model"])["threshold"]
spark_thresholds = pd.read_csv(args.spark_results).set_index(["skill", "model"])["threshold"]
...
spark_metrics = pd.read_csv(args.spark_results)[["skill", "model", "pr_auc", "roc_auc"]]
nonspark_metrics = pd.read_csv(args.nonspark_results)[["skill", "model", "pr_auc", "roc_auc"]]
