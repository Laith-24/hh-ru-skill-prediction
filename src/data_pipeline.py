...
# Columns of the transformed frame that come from the job title alone (the seniority flags are read from the title text).
TITLE_FEATURE_COLS = ["seniority_vector", "title_tfidf", "title_w2v"]
...
# (?U) makes \W Unicode-aware. Java's plain \W is ASCII-only, so without it every
# Cyrillic letter counts as a separator and Russian text produces no tokens at all.
TOKEN_PATTERN = r"(?U)\W+"


def _skill_name_tokens(top_skills):
    """Tokenize skill names the same way RegexTokenizer splits description
    text (Unicode \\W+, lowercased, min length 2), so they can be excluded from the
...
def spark_save_path(path: str, k8s: bool) -> str:
    ...
    simply isn't where the path actually lives. Prefixing
...
def spark_load_with_retry(load_fn, what, attempts=5, delay=10):
    ...
    Spark-native sweep). train_spark_native.py's own
...
def build_ml_pipeline_no_leakage(top_employers, top_skills):
    ...
    Confirmed empirically: with the leaky pipeline, every
    ...
    is real signal, not the label leaking into the features.
    ...
    # "Менеджер по продажам") -- a Unicode-aware word-boundary tokenizer plus
    # combined RU+EN stopword lists handles both without extra NLP deps.
    title_tokenizer = RegexTokenizer(
        inputCol="name", outputCol="title_tokens_raw", pattern=TOKEN_PATTERN, toLowercase=True, minTokenLength=2,
    )
    ...
    desc_tokenizer = RegexTokenizer(
        inputCol="description_clean", outputCol="desc_tokens_raw", pattern=TOKEN_PATTERN, toLowercase=True, minTokenLength=2,
    )
    desc_stopwords = (
...
def _get_skill_list(df, top_n_skills, skill_list_path):
    ...
    if path.exists():
        ...
        if len(skills) != top_n_skills:
            logger.warning(f"{path} holds {len(skills)} skills but {top_n_skills} were requested; "
                           f"using the cached list. Pass another skill_list_path to build a list of a different size.")
